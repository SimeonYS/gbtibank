...
class GgbtibankPipeline:

    # Database setup
    conn = sqlite3.connect('gbtibank.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `gbtibank`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM gbtibank WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO gbtibank (date, title, link, content)"
